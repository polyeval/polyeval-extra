...
from polyeval_extra.target.emacslisp.value import EmacsLispTargetValue
...
class EmacsLispTargetTests(BaseTargetTests):
    def __init__(self):
        self.naming = EmacsLispTargetNaming()
        self.type = EmacsLispTargetType()
        self.value = EmacsLispTargetValue()
...
"""
    ...
    def gen_input_str_cmd(self, params_names: list[str], input_types: list[Type]):
        ...
        return f'(setq i-str (concat "input: (" {val_str} ")\n"))'

    def gen_side_str_cmd(self, params_names: list[str], input_types: list[Type]):
        val_str = ' ", " '.join(
            [self.stringify.get(t, name) for name, t in zip(params_names, input_types)]
        )
        return f'(setq s-str (concat "side-effect: (" {val_str} ")\n"))'
