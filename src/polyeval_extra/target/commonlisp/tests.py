...
from polyeval_extra.target.commonlisp.value import CommonLispTargetValue
...
class CommonLispTargetTests(BaseTargetTests):
    def __init__(self):
        self.naming = CommonLispTargetNaming()
        self.type = CommonLispTargetType()
        self.value = CommonLispTargetValue()
    ...
    def gen_code(self, funcs: list[Function]):
        ...
        test_entry = f"""\
(defun p_e_entry ()
    (concatenate 'string {test_entry_ret}))
...
"""
    ...
    def gen_function_test(
        ...
        test_code = f"""\
(defun {test_item_func_name} ()
    {add_indent(self.gen_assign_vars_cmds(params_names, test_inputs), 0)}
...
"""
    ...
    def gen_assign_vars_cmds(
        ...
        for name, value in zip(params_names, test_inputs):
            result.append(f"(setq {name} {self.value.by(value)})")
        return "\n".join(result)
