...
from polyeval_extra.target.standardml.value import StandardMLTargetValue
...
class StandardMLTargetTests(BaseTargetTests):
    def __init__(self):
        self.naming = StandardMLTargetNaming()
        self.type = StandardMLTargetType()
        self.value = StandardMLTargetValue()
...
"""
    ...
    def gen_function_test(
        ...
        test_code = f"""\
val {test_item_func_name} =
    let
        {add_indent(self.gen_assign_vars_cmds(params_names, test_inputs), 2)}
...
"""
    ...
    def gen_assign_vars_cmds(
        ...
        for name, value in zip(params_names, test_inputs):
            result.append(f"val {name} = {self.value.by(value)};")
        return "\n".join(result)
